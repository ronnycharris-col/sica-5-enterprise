...
for (const registro of registros) {
    ...
    if (!empleados.has(documento)) {
        empleados.set(documento, []);
    }

    empleados.get(documento).push(registro);
...
for (const registrosEmpleado of empleados.values()) {

    registrosEmpleado.sort(
        (a, b) => a.fechaServidor.seconds - b.fechaServidor.seconds
    );

    for (let i = 0; i < registrosEmpleado.length; i++) {

        const entrada = registrosEmpleado[i];
        ...
        for (let j = i + 1; j < registrosEmpleado.length; j++) {

            if (registrosEmpleado[j].tipo === "Salida") {

                salida = registrosEmpleado[j];
                i = j;
                break;
